...
export async function getAvatarVoiceForLanguage(
  ...
  const defaults = DEFAULT_VOICES[targetLanguage]
  const candidates = [
  ...
  if (!resolvedVoiceId) {
    throw new Error(
      `No ElevenLabs voice configured for language ${targetLanguage}. Please set ELEVENLABS_DEFAULT_VOICE_${targetLanguage.toUpperCase()}.`
    )
  }
